...
- **两套 API 配置的 api_url 格式约定不同，生图侧自动剥尾部 `/v1`**（2026-08-19，换电脑部署实测踩坑）：AI 分析配置（`config.json`）的 api_url **需带 `/v1` 后缀**——`ai_client` 只拼最后一段（`/chat/completions` 或 `/messages`），遵循 OpenAI 生态 base_url 惯例、兼容非标路径代理；生图配置（`gen_config.json`）的 api_url 是**服务器根地址不带 `/v1`**——`image_gen_client` 按 api_type 拼完整路径（`/v1/images/...` 或 `/api/openapi/...`），aireiter 路径没有 `/v1` 层级所以不能让用户带。跨电脑部署时照抄分析地址到生图是高频错误，且两套的 api_key 也不是同一把——表现为"分析正常、生图全部 500"（拼成 `/v1/v1/...` 全 404）。防呆：`ImageGenClient.__init__` 对尾部 `/v1` 自动剥离（后面必拼 `/v1/...`，尾部 `/v1` 必然是填错，零风险纠错；只匹配 `/v1` 结尾不误伤 `/v10`），两个设置组件的 URL 输入框下加了格式提示小字。**分析侧不做自动补 `/v1`**——存在真的没有 v1 层级的代理，自动补会破坏它们，只靠提示文案
